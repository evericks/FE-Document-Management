...
export class OrganizationHeaderComponent implements OnInit {
        ...
        this._fuseConfirmationService.open({
            ...
            if (result === 'confirmed') {
                if (data.isNew) {
                    ...
                    this._organizationService.deleteOrganization(data.id).subscribe(() => {
                        this.gridApi.applyTransaction({ remove: [data] });
                        this.rowData.shift();
                    });
                }
    ...
    onCellValueChanged(data) {
        if (!data.isNew) {
            this._organizationService.updateOrganization(data.id, data).subscribe();
        }
    }

    onSaveButtonClicked(data) {
        this._organizationService.createOrganization(data).subscribe();
    }
}
